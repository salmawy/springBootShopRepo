...
public class SupplierPersenter extends ContractorAction implements Initializable {
	...
	private void init() {
		...
		contractorPredicatableTable = new PredicatableTable<ContractorVB>(contractorColumns, contratorHeadNodes, null,
				new ContractorableActionListner(), PredicatableTable.headTableCard, ContractorVB.class);
		...
		try {
			...
			for (Iterator iterator = transactions.iterator(); iterator.hasNext();) {
				...
				viewBean.setPaid((transaction.getPaid() == 1)
						? getAppContext().getMessages().getString("label.contractor.status.paid.yes")
