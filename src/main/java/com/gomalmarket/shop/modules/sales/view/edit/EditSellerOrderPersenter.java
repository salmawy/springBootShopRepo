...
//====================================================================================================
		...
		if (validateInputData()) {
			...
			try {
				...
				order.setOrderWeights(orerDetail);
 				if (this.orderId != 0)
					this.getSalesService().editeSellerOrder(seller, order, paidAmount_, this.oldOrder,
							getAppContext().getSeason().getId());
				else
					 order=this.getSalesService().saveSellerOrder(seller, order, paidAmount_);

				clear();
			    String msg=	(this.orderId == 0)?"msg.done.save":"msg.done.edit";
				
				
				this.response=new HashMap<String, Object>();
				response.put(Response.RESPONSE_CODE_KEY,NavigationResponseCodeEnum.SUCCESS);
				response.put("order",order);
				
				
			    alert(AlertType.INFORMATION, "", "", this.getMessage(msg));
	...
	private List<JFXButton> prepareOrderDetailcontrolles() {
		...
		deleteBtn.setOnAction(e -> {
			...
			try {
				...
				CustomerOrder customerOrder = (CustomerOrder) this.getBaseService().findBean(CustomerOrder.class,
						selectedItem.getCustomerOrderId());
	...
	private void addOrderDetail() {
		...
		Scene scene1 = new Scene(form.getView(), 400, 450);
	...
	private void updateTotalAmountValue() {
	    List <SellerOrderDetailVB> ordeweights= this.orderDetail_CT.getTable().getItems();
		
		double  value=ordeweights.stream().mapToDouble(e->e.getAmount()).sum();		
	...
	private void trackPaidValue() {
		if (!totalAmount.getText().isEmpty()) {
			...
			if (total != 0) {
				...
				if (index == SellerTypeEnum.cash.getId()) {
...
//==================================================================================
			if (oldOrder.getSeller().getTypeId() == 1) {
				...
				this.datePicker
						.setValue(this.getBaseService().convertToLocalDateViaMilisecond(oldOrder.getOrderDate()));
				// ==================================================================================
