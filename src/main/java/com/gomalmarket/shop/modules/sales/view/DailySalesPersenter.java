...
package com.gomalmarket.shop.modules.sales.view;
...
public class DailySalesPersenter extends SalesAction implements Initializable, CustomTableActions {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		edit_btn.setOnAction(e -> {
			SellerOrderVB orderVB = (SellerOrderVB) this.sellerOrdersCustomTable.getTable().getSelectionModel()
					.getSelectedItem();
		...
		sellerOrdersCustomTable = new CustomTable<SellerOrderVB>(sellerOrderColumns, null, null, null, this,
				CustomTable.tableCard, SellerOrderVB.class);
		sellerOrderDetailsCustomTable = new CustomTable<SellerOrderDetailVB>(SellerOrderDetailColumns, null, null, null,
				null, CustomTable.tableCard, SellerOrderDetailVB.class);
		orderDetail_CT = new CustomTable<SellerOrderDetailVB>(SellerOrderDetailColumns, orderDetailControles, null,
				null, null, CustomTable.headTableCard, SellerOrderDetailVB.class);
		...
		sellerType_CB.getItems()
				.add(new ComboBoxItem(SellerTypeEnum.cash.getId(), this.getMessage("seller.type.cash")));
		sellerType_CB.getItems()
				.add(new ComboBoxItem(SellerTypeEnum.permenant.getId(), this.getMessage("seller.type.permenant")));
	...
	private List<JFXButton> prepareOrderDetailcontrolles() {
		...
		deleteBtn.setOnAction(e -> {
			SellerOrderDetailVB selectedItem = (SellerOrderDetailVB) orderDetail_CT.getTable().getSelectionModel()
					.getSelectedItem();
	...
	private void addOrderDetail() {
		...
		Scene scene1 = new Scene(form.getView(), 400, 420);
	...
	private void updateTotalAmountValue(double value, int mode) {
		...
		double oldValue = (totalAmount.getText() != null && totalAmount.getText() != ""
				&& totalAmount.getText().length() > 0) ? Double.parseDouble(totalAmount.getText()) : 0.0;
	...
	private void loadOrderDetail(int orderId) {
		...
		try {
			...
			for (Iterator iterator = weights.iterator(); iterator.hasNext();) {
				...
				viewBean.setCustomerOrderId(weight.getCustomerOrder().getDued());
	...
	private void loadBookSellerOrders(Date date) {
		...
		try {
			...
			for (Iterator iterator = result.iterator(); iterator.hasNext();) {
				...
				String sellerTypeText = (order.getSeller().getTypeId() == SellerTypeEnum.cash.getId())
						? this.getMessage("seller.type.cash")
						: this.getMessage("seller.type.permenant");
	...
	private double getPaidAmount(int orderId) {
		...
		try {
			IncomeDetail income = (IncomeDetail) this.getBaseService().findAllBeans(IncomeDetail.class, map, null)
					.get(0);
	...
	private void addEditOrder(int orderId) {
		...
		Scene scene1 = new Scene(form.getView(), 1200, 500);
		Stage popupwindow = new Stage();
		...
		popupwindow.setOnHiding(ev -> {
			...
			if (response != null && response.get(Response.RESPONSE_CODE_KEY) != null
					&& response.get(Response.RESPONSE_CODE_KEY) == NavigationResponseCodeEnum.SUCCESS) {

				SellerOrder order = (SellerOrder) response.get("order");
		//		this.loadBookSellerOrders(order.getOrderDate());
				this.datePicker.setValue(getBaseService().convertToLocalDateViaMilisecond(order.getOrderDate()));
