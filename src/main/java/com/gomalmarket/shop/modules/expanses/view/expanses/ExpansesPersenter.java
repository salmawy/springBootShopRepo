...
//---------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		incomeCustomTable = new CustomTable<IncomeVB>(incomeColumns, null, null, null, null, CustomTable.tableCard,
				IncomeVB.class);
		outcomeCustomTable = new CustomTable<OutcomeVB>(outcomeColumns, null, null, null, this, CustomTable.tableCard,
				OutcomeVB.class);
		...
		double balance = this.getExpansesServices().getSafeBalanceOfday(getAppContext().getSeason().getId(), new Date(),
				SafeTypeEnum.OPENNNING_BALANCE);
		initailBalanceValue_label.setText(String.valueOf(balance));
		log.info("safe intialbalance=" + balance);

		balance = this.getExpansesServices().getSafeBalanceOfday(getAppContext().getSeason().getId(), new Date(),
				SafeTypeEnum.CURRENT_BALANCE);
		currentBalanceValue_label.setText(String.valueOf(balance));
		log.info("safe current Balance=" + balance);
...
// loadOutcomeData(day.getId());			
		...
		try {
			...
			for (Iterator iterator = data.iterator(); iterator.hasNext();) {
				...
				if (detail.getTypeId() == IncomeTypeEnum.INTST_PAY.getId()) {
					...
					name = seller.getName();
				} else if (detail.getTypeId() == IncomeTypeEnum.IN_LOAN.getId()
						|| detail.getTypeId() == IncomeTypeEnum.PAY_CREDIT.getId()) {
	...
	private void fillOutcomeMonthes() {
		...
		try {
			...
			List result = this.getBaseService().findAllBeans(Outcome.class, map,
					Arrays.asList(new JPAOrderBy("outcomeDate", JPAOrderByEnum.DESC)));
...
//-------------------------------------------------------------------------------------------------------------------------
	...
	private void loadOutcomeData(int outcomeId) {
		...
		try {
			...
			for (Iterator iterator = data.iterator(); iterator.hasNext();) {
				...
				String orderTag = (detail.getOrderId() == null || detail.getOrderId() == -1) ? ""
						: String.valueOf(detail.getOrderId());
				...
				if (detail.getCustomerId() != -1) {
					...
					if (detail.getTypeId() == OutcomeTypeEnum.K_L.getId()
							|| detail.getTypeId() == OutcomeTypeEnum.K_V.getId()
							|| detail.getTypeId() == OutcomeTypeEnum.K_S.getId())

					{
						Contractor contractor = (Contractor) this.getBaseService().getBean(Contractor.class,
								detail.getCustomerId());
						name = contractor.getName();

					} else if (detail.getTypeId() == OutcomeTypeEnum.OUT_LOAN.getId()
							|| detail.getTypeId() == OutcomeTypeEnum.PAY_DEBIT.getId()) {
						...
						Customer customer = (Customer) this.getBaseService().getBean(Customer.class,
								detail.getCustomerId());
	...
	private void prepareOutcomeHeaderNodes() {
		...
		List nodes = new ArrayList(Arrays.asList(addOutcome_btn, editOutcome_btn, tempBox, outcomeDay_CB,
				outcomeMonth_CB, outcomeTotalAmount_label, outcomeTotalAmountValue_label));
	...
	private void setIncomDayseParentKey(String key) {
		...
		List<ComboBoxItem<Integer>> result = incomeDays.stream().filter(day -> day.getParentKey().equals(key))
	...
	private void setOutcomDayseParentKey(String key) {
		...
		List<ComboBoxItem<Integer>> result = outcomeDays.stream().filter(day -> day.getParentKey().equals(key))
				.collect(Collectors.toList());
